...
public class PartyInviteListener implements Listener{
	...
	public void onInvite(PartyInviteEvent e){
		...
		TextComponent prefix = new TextComponent(Main.prefix + ChatColor.GRAY + "Du hast eine Party anfrage von " + ChatColor.GOLD + e.getHost().getName() + ChatColor.GRAY + " bekommen");
		...
		message.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/p accept " + e.getHost().getName()));
		...
		message2.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/p decline " + e.getHost().getName()));
